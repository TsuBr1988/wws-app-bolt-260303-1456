...
interface EditContractFormProps {
  ...
  onClose: () => void;
  onSubmit: (contractId: string, updates: any) => Promise<void>;
}

export function EditContractForm({ contract, onClose, onSubmit }: EditContractFormProps) {
  const [formData, setFormData] = useState({
    client_name: contract.client_name,
    empresa: contract.empresa,
    tipo: contract.tipo || 'Publico',
    city: contract.city || '',
    ...
    prazo_maximo_renovacao: (contract as any).prazo_maximo_renovacao?.toString() || '',
  });
  ...
  useEffect(() => {
    const changed =
      ...
      formData.empresa !== contract.empresa ||
      formData.tipo !== (contract.tipo || 'Publico') ||
      formData.city !== (contract.city || '') ||
      formData.numero_pregao !== (contract.numero_pregao || '') ||
      formData.numero_contrato !== (contract.numero_contrato || '') ||
      formData.monthly_value !== contract.monthly_value.toString() ||
      ...
      formData.prazo_maximo_renovacao !== ((contract as any).prazo_maximo_renovacao?.toString() || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.prazo_maximo_renovacao) {
        updateData.prazo_maximo_renovacao = parseInt(formData.prazo_maximo_renovacao);
      } else {
        updateData.prazo_maximo_renovacao = null;
      }

      await onSubmit(contract.id, updateData);
    } catch (error) {
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="font-medium text-yellow-900 mb-1">
                  Este contrato possui aditivos
                </p>
                <p className="text-sm text-yellow-700">
                  As alterações afetam apenas os dados originais do contrato. O valor e data atuais continuam sendo determinados pelos aditivos.
                </p>
              </div>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {hasChanges && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <h4 className="font-semibold text-blue-900 mb-3">Resumo das Alterações</h4>
