...
export const CapexModal = ({ isOpen, onClose, onSave, capex, budgetFunctions = [] }: CapexModalProps) => {
  const [formData, setFormData] = useState<CapexData>({
    name: '',
    brand: '',
    quantity: 1,
    unit_value: 0,
  ...
  const [configCapex, setConfigCapex] = useState<ConfigCapex[]>([]);
  ...
  useEffect(() => {
    ...
    if (capex) {
      setFormData({
        ...capex,
        allocated_functions: capex.allocated_functions || [],
      });
    } else {
      setFormData({
        name: '',
  ...
  const loadConfigCapex = async () => {
    ...
    try {
      ...
      console.error('Error loading config capex:', error);
    } finally {
      setLoadingConfig(false);
    }
  };

  const handleSelectConfigCapex = (configId: string) => {
    if (!configId) return;

    const config = configCapex.find(c => c.id === configId);
    if (!config) return;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      alert('Por favor, informe o nome do item de Capex');
      return;
    }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-slate-800">
            {capex ? 'Editar Capex' : 'Incluir Capex'}
          </h2>
          <button
            onClick={onClose}
          ...
          <div className="space-y-4">
            {!capex && (
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
                <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
                  ...
                  Selecionar Capex Pré-Cadastrado (Opcional)
                </label>
                <select
                  onChange={(e) => handleSelectConfigCapex(e.target.value)}
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Servidor de monitoramento"
              />
            </div>
              ...
              <div className="bg-slate-50 border border-slate-300 rounded-lg p-4">
                <div className="mb-3">
                  <h3 className="text-sm font-semibold text-slate-800 mb-1">
                    Alocar Capex nas Funções
                  </h3>
                  <p className="text-xs text-slate-600">
                    Selecione as funções que receberão o rateio deste investimento. Se nenhuma for selecionada, o valor será rateado entre todas as funções.
                  </p>
                </div>
