...
import { UniformsTableRow, PurchasesUniforms } from '@/types/database';
import { useToast } from '@/components/ui/use-toast';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, TooltipProps } from 'recharts';

interface UniformsChartData {
  monthLabel: string;
  orcado: number;
  gasto: number;
...
export function UniformsCard() {
  const [loading, setLoading] = useState(true);
  const [tableData, setTableData] = useState<UniformsTableRow[]>([]);
  const [chartData, setChartData] = useState<UniformsChartData[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await DashboardService.getPurchasesUniforms(monthsRange);

      const contractsMap = new Map<string, UniformsTableRow>();
      ...
      const chartDataByMonth = months.map((month) => {
        ...
        const chartPoint: UniformsChartData = {
      ...
      toast({
        ...
        description: 'Não foi possível carregar os dados de uniformes.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const updateSingleMonthChart = (data: PurchasesUniforms[], monthYm: string) => {
  ...
  useEffect(() => {
    if (selectedMonth && tableData.length > 0) {
      DashboardService.getPurchasesUniforms(months.map(m => m.monthYm))
  ...
  const updateYearlyChart = (data: PurchasesUniforms[], year: string, contractFilter: string) => {
  ...
  const handleSave = async (data: UniformsTableRow[], deletedRows: UniformsTableRow[]) => {
    try {
      const records: Omit<PurchasesUniforms, 'id' | 'created_at' | 'updated_at'>[] = [];
      ...
      data.forEach((row, index) => {
        if (!row.contract_name || !row.contract_name.trim()) {
          ...
          console.log(`Row ${index} has empty contract name`);
          return;
        }

        if (!row.company) {
          hasInvalidData = true;
          console.log(`Row ${index} has empty company`);
          return;
        }

        months.forEach((month) => {
          const amountSpent = Number(row[month.monthYm]) || 0;
          const budget = Number(row.budget_2025) || 0;
      ...
      if (hasInvalidData) {
        toast({
          title: 'Erro de validação',
          description: 'Empresa não pode estar vazia.',
          variant: 'destructive',
        });
        return;
      }
      ...
      if (deletedRows.length > 0) {
        await DashboardService.deletePurchasesUniforms(deletedRows);
      }

      if (records.length > 0) {
        await DashboardService.upsertPurchasesUniforms(records);
      }
