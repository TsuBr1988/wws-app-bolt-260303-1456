...
import { ContractMarginTable } from '@/components/dashboard/ContractMarginTable';
import { DashboardService } from '@/services/dashboardService';
import { getLast12Months } from '@/lib/months';
import { ContractMarginTableRow, FinancialContractMargin } from '@/types/database';
import { useToast } from '@/components/ui/use-toast';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, TooltipProps } from 'recharts';

interface ContractMarginChartData {
  monthLabel: string;
  orcado: number;
  gasto: number;
...
export function ContractMarginCard() {
  const [loading, setLoading] = useState(true);
  const [tableData, setTableData] = useState<ContractMarginTableRow[]>([]);
  const [chartData, setChartData] = useState<ContractMarginChartData[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await DashboardService.getFinancialContractMargin(monthsRange);

      const contractsMap = new Map<string, ContractMarginTableRow>();
      ...
      const chartDataByMonth = months.map((month) => {
        ...
        const chartPoint: ContractMarginChartData = {
      ...
      toast({
        ...
        description: 'Não foi possível carregar os dados de margem de contribuição.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const updateSingleMonthChart = (data: FinancialContractMargin[], month: string) => {
  ...
  useEffect(() => {
    if (selectedMonth && tableData.length > 0) {
      DashboardService.getFinancialContractMargin(months.map(m => m.monthYm))
  ...
  useEffect(() => {
    if (selectedYear && tableData.length > 0) {
      DashboardService.getFinancialContractMargin(months.map(m => m.monthYm))
        .then(data => updateYearlyChart(data, selectedYear))
        .catch(console.error);
    }
  }, [selectedYear]);

  const updateYearlyChart = (data: FinancialContractMargin[], year: string) => {
  ...
  const handleSave = async (data: ContractMarginTableRow[]) => {
    try {
      const records: Omit<FinancialContractMargin, 'id' | 'created_at' | 'updated_at'>[] = [];
      ...
      await DashboardService.upsertFinancialContractMargin(records);
      await loadData();
  ...
  return (
    <IndicatorCard
      title="Margem de Contribuição Contratos"
