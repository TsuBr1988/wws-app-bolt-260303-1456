...
import { CleaningMaterialsTable } from '@/components/dashboard/CleaningMaterialsTable';
import { DashboardService } from '@/services/dashboardService';
import { getLast12Months } from '@/lib/months';
import { CleaningMaterialsTableRow, PurchasesCleaningMaterials } from '@/types/database';
import { useToast } from '@/components/ui/use-toast';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, TooltipProps } from 'recharts';

interface CleaningMaterialsChartData {
  monthLabel: string;
  orcado: number;
  gasto: number;
...
export function CleaningMaterialsCard() {
  const [loading, setLoading] = useState(true);
  const [tableData, setTableData] = useState<CleaningMaterialsTableRow[]>([]);
  const [chartData, setChartData] = useState<CleaningMaterialsChartData[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await DashboardService.getPurchasesCleaningMaterials(monthsRange);

      const contractsMap = new Map<string, CleaningMaterialsTableRow>();
      ...
      const chartDataByMonth = months.map((month) => {
        ...
        const chartPoint: CleaningMaterialsChartData = {
      ...
      toast({
        ...
        description: 'Não foi possível carregar os dados de materiais de limpeza.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const updateSingleMonthChart = (data: PurchasesCleaningMaterials[], monthYm: string) => {
  ...
  useEffect(() => {
    if (selectedMonth && tableData.length > 0) {
      DashboardService.getPurchasesCleaningMaterials(months.map(m => m.monthYm))
  ...
  const updateYearlyChart = (data: PurchasesCleaningMaterials[], year: string, contractFilter: string) => {
  ...
  const handleSave = async (data: CleaningMaterialsTableRow[], deletedRows: CleaningMaterialsTableRow[]) => {
    try {
      const records: Omit<PurchasesCleaningMaterials, 'id' | 'created_at' | 'updated_at'>[] = [];
      ...
      if (deletedRows.length > 0) {
        await DashboardService.deletePurchasesCleaningMaterials(deletedRows);
      }

      await DashboardService.upsertPurchasesCleaningMaterials(records);
      await loadData();
