...
import { SemPararTable } from '@/components/dashboard/SemPararTable';
import { DashboardService } from '@/services/dashboardService';
import { getLast12Months } from '@/lib/months';
import { SemPararTableRow, PurchasesSemParar } from '@/types/database';
import { useToast } from '@/components/ui/use-toast';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, TooltipProps } from 'recharts';

interface SemPararChartData {
  monthLabel: string;
  WWS: number;
  Worldwide: number;
}
...
export function SemPararCard() {
  const [loading, setLoading] = useState(true);
  const [tableData, setTableData] = useState<SemPararTableRow[]>([]);
  const [chartData, setChartData] = useState<SemPararChartData[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await DashboardService.getPurchasesSemParar(monthsRange);

      const departmentsMap = new Map<string, SemPararTableRow>();
      ...
      toast({
        ...
        description: 'Não foi possível carregar os dados de Sem Parar.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const updateChartData = (data: PurchasesSemParar[]) => {
    ...
    const chartDataByMonth = months.map((month) => {
      ...
      const chartPoint: SemPararChartData = {
  ...
  const updateSingleMonthChart = (data: PurchasesSemParar[], monthYm: string) => {
  ...
  useEffect(() => {
    if (tableData.length > 0) {
      DashboardService.getPurchasesSemParar(months.map(m => m.monthYm))
  ...
  const updateYearlyChart = (data: PurchasesSemParar[], year: string, departmentFilter: string) => {
  ...
  const handleSave = async (data: SemPararTableRow[], deletedRows: SemPararTableRow[]) => {
    try {
      const records: Omit<PurchasesSemParar, 'id' | 'created_at' | 'updated_at'>[] = [];
      ...
      if (deletedRows.length > 0) {
        await DashboardService.deletePurchasesSemParar(deletedRows);
      }

      if (records.length > 0) {
        await DashboardService.upsertPurchasesSemParar(records);
      }
      ...
      console.error('Error saving sem parar data:', error);
      toast({
