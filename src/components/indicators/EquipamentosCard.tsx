...
import { EquipamentosTableRow, PurchasesEquipamentos } from '@/types/database';
import { useToast } from '@/components/ui/use-toast';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, TooltipProps } from 'recharts';

interface EquipamentosChartData {
  monthLabel: string;
  orcado: number;
  gasto: number;
...
export function EquipamentosCard() {
  const [loading, setLoading] = useState(true);
  const [tableData, setTableData] = useState<EquipamentosTableRow[]>([]);
  const [chartData, setChartData] = useState<EquipamentosChartData[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await DashboardService.getPurchasesEquipamentos(monthsRange);

      const contractsMap = new Map<string, EquipamentosTableRow>();
      ...
      const chartDataByMonth = months.map((month) => {
        ...
        const chartPoint: EquipamentosChartData = {
      ...
      toast({
        ...
        description: 'Não foi possível carregar os dados de equipamentos.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const updateSingleMonthChart = (data: PurchasesEquipamentos[], monthYm: string) => {
  ...
  useEffect(() => {
    if (selectedMonth && tableData.length > 0) {
      DashboardService.getPurchasesEquipamentos(months.map(m => m.monthYm))
  ...
  const updateYearlyChart = (data: PurchasesEquipamentos[], year: string, contractFilter: string) => {
    ...
    yearRecords.forEach(record => {
      ...
      if (!contractMap.has(key)) {
        contractMap.set(key, { orcado: Number(record.budget_2025), gasto: 0 });
      }
    ...
    const chartData = Array.from(contractMap.entries()).map(([contract_name, values]) => ({
      contract_name, orcado: values.orcado, gasto: values.gasto
    }));
    chartData.sort((a, b) => b.gasto - a.gasto);
    setYearlyChartData(chartData);
  };

  const handleSave = async (data: EquipamentosTableRow[], deletedRows: EquipamentosTableRow[]) => {
    try {
      const records: Omit<PurchasesEquipamentos, 'id' | 'created_at' | 'updated_at'>[] = [];
      let hasEmptyContract = false;
      ...
      if (deletedRows.length > 0) {
        await DashboardService.deletePurchasesEquipamentos(deletedRows);
      }

      if (records.length > 0) {
        await DashboardService.upsertPurchasesEquipamentos(records);
      }
      ...
      console.error('Error saving equipamentos data:', error);
      toast({
