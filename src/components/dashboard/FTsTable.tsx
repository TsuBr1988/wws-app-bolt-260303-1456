...
import { FTsTableRow, Client } from '@/types/database';

interface FTsTableProps {
  data: FTsTableRow[];
  months: { monthYm: string; monthLabel: string }[];
  clients: Client[];
  onSave: (data: FTsTableRow[], deletedRows: { company: string; contract_name: string }[]) => Promise<void>;
  loading: boolean;
}

export function FTsTable({ data, months, clients, onSave, loading }: FTsTableProps) {
  const [tableData, setTableData] = useState<FTsTableRow[]>(data);
  ...
  const handleAddRow = () => {
    setTableData([
      ...
      {
        company: 'WWS',
        contract_name: clients.length > 0 ? clients[0].name : '',
        budget_2025: 0,
      },
    ]);
  };
