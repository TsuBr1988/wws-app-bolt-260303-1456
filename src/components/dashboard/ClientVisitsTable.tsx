...
import { ClientVisitsTableRow } from '@/types/database';
import { DashboardService } from '@/services/dashboardService';

interface ClientVisitsTableProps {
  data: ClientVisitsTableRow[];
  months: { monthYm: string; monthLabel: string }[];
  onSave: (data: ClientVisitsTableRow[], deletedRows: ClientVisitsTableRow[]) => Promise<void>;
  loading: boolean;
}

export function ClientVisitsTable({ data, months, onSave, loading }: ClientVisitsTableProps) {
  const [tableData, setTableData] = useState<ClientVisitsTableRow[]>(data);
  const [deletedRows, setDeletedRows] = useState<ClientVisitsTableRow[]>([]);
  ...
  const handleDeleteRow = (rowIndex: number) => {
    ...
    if (rowToDelete.contract_name && rowToDelete.contract_name.trim() !== '') {
