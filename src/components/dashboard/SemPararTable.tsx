...
import { SemPararTableRow } from '@/types/database';

interface SemPararTableProps {
  data: SemPararTableRow[];
  months: { monthYm: string; monthLabel: string }[];
  onSave: (data: SemPararTableRow[], deletedRows: SemPararTableRow[]) => Promise<void>;
  loading: boolean;
}

export function SemPararTable({ data, months, onSave, loading }: SemPararTableProps) {
  const [tableData, setTableData] = useState<SemPararTableRow[]>(data);
  const [deletedRows, setDeletedRows] = useState<SemPararTableRow[]>([]);
  ...
  const handleCellChange = (rowIndex: number, key: string, value: string) => {
    ...
    newData[rowIndex] = {
      ...newData[rowIndex],
      [key]: key === 'company' || key === 'department' ? value : Number(value) || 0,
    };
