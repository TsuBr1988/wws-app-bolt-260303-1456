...
import { CombustivelTableRow } from '@/types/database';

interface CombustivelTableProps {
  data: CombustivelTableRow[];
  months: { monthYm: string; monthLabel: string }[];
  onSave: (data: CombustivelTableRow[], deletedRows: CombustivelTableRow[]) => Promise<void>;
  loading: boolean;
}

export function CombustivelTable({ data, months, onSave, loading }: CombustivelTableProps) {
  const [tableData, setTableData] = useState<CombustivelTableRow[]>(data);
  const [deletedRows, setDeletedRows] = useState<CombustivelTableRow[]>([]);
