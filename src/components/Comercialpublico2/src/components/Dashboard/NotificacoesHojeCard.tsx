...
import { Bell, ChevronDown, ChevronRight, Clock, User, AlertTriangle } from 'lucide-react';
import { useNotificacoesHoje } from '../../hooks/useNotificacoesDashboard';
...
export const NotificacoesHojeCard: React.FC = () => {
  const [expanded, setExpanded] = useState(false);
  const [selectedItem, setSelectedItem] = useState<any | null>(null);
  const [showModal, setShowModal] = useState(false);
  const { items, loading, error } = useNotificacoesHoje();
        ...
        >
          <div>
            <h3 className="text-lg font-semibold text-gray-900 flex items-center space-x-2">
              ...
              <span>Notificações Vencendo Hoje</span>
            </h3>
            <p className="text-sm text-gray-600 text-left">Notificações e ofícios com prazo hoje</p>
          </div>
          <div className="flex items-center space-x-3">
            <div className="text-right">
              <div className="text-2xl font-bold text-red-600">{items.length}</div>
              <div className="hidden md:block text-sm text-gray-500">Hoje</div>
            </div>
            {expanded ? (
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {loading ? (
                ...
                items.slice(0, 10).map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full bg-red-50 border border-red-200 border-l-4 border-l-red-600 rounded-lg p-4 hover:bg-red-100 transition-colors text-left"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right ml-3">
                          <div className="space-y-1">
                            <div className={`px-2 py-1 rounded-full text-xs font-medium border ${deadlineBadge.cls}`}>
                              {deadlineBadge.label}
                            </div>
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Nenhuma notificação vencendo hoje!</h3>
                  <p className="text-gray-500">Não há notificações com prazo para hoje</p>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-4 text-center text-sm">
                  <div>
                    <div className="text-lg font-bold text-yellow-600">
                      {items.filter(n => n.situacao === 'A fazer').length}
                    </div>
                    <div className="text-gray-600">A fazer</div>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-gray-600">
                      {items.filter(n => n.situacao === 'Não iremos responder').length}
                    </div>
                    <div className="text-gray-600">Não responder</div>
                  </div>
                </div>
              </div>
            )}
