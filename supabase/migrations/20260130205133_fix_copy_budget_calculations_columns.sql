/*
  # Fix copy_budget_as_new function - Remove invalid columns from budget_calculations copy

  1. Changes
    - Update the function to not copy budget_calculations columns that don't exist
    - Remove references to total_payroll, total_benefits, total_encargos and other columns
    
  2. Notes
    - The budget_calculations table structure has changed, so we need to update the copy function
*/
...
BEGIN
  ...
  -- NÃO COPIA budget_calculations pois é uma tabela de cache que será recalculada automaticamente
