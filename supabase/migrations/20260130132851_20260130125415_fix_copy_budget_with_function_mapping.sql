/*
  # CORREÇÃO FINAL: Função copy_budget_as_new com mapeamento de funções

  ## Problema Identificado
  A tabela `uniforms` tem coluna `function_id NOT NULL` que precisa ser mapeada
  corretamente ao copiar de um orçamento para outro.

  ## Solução
  Criar um mapeamento temporário entre IDs de funções antigas e novas durante a cópia.
...
BEGIN
  ...
  FROM uniforms WHERE budget_id = p_original_budget_id;
  ...
  SELECT
    ...
    name,
    monthly_value
  FROM differentiated_benefits WHERE budget_id = p_original_budget_id;
