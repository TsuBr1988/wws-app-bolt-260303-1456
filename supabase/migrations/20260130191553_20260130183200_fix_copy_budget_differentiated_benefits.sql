/*
  # Fix copy_budget_as_new function - Correct differentiated_benefits columns

  1. Changes
    - Fix differentiated_benefits copy to use correct columns: name, monthly_value
    - Fix function_id type conversion (text instead of uuid)
    - Also fix uniforms function_id conversion to text
*/
...
BEGIN
  ...
  SELECT COALESCE(MAX(sequence_number), 0) + 1 INTO v_new_sequence 
  FROM budgets WHERE year = v_current_year;
  
  v_new_budget_number := 'ORC-' || v_current_year || '-' || LPAD(v_new_sequence::text, 3, '0');

  -- Criar novo orçamento
  INSERT INTO budgets (
    budget_number, sequence_number, client_name, description, name, vt_value, iss_rate, city,
    service_type, status, year, contact_name, contact_email, contact_phone, margem_lucro,
    margem_adm, city_name, cnpj, email, phone, address, lead_source
  ) VALUES (
    v_new_budget_number, v_new_sequence,
    COALESCE(p_new_client_name, v_original_budget.client_name),
    COALESCE(p_new_description, v_original_budget.description),
    v_original_budget.name, v_original_budget.vt_value, v_original_budget.iss_rate,
    v_original_budget.city, v_original_budget.service_type, 'open', v_current_year,
    v_original_budget.contact_name, v_original_budget.contact_email,
    v_original_budget.contact_phone, v_original_budget.margem_lucro,
  ...
  -- Copiar budget_function_benefit_overrides
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'budget_function_benefit_overrides') THEN
    ...
    WHERE budget_id = p_original_budget_id::text;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'equipments') THEN
    ...
    SELECT v_new_budget_id, name, brand, quantity, unit_value, total_value, amortization, monthly_value, allocated_functions, function_id
    FROM equipments WHERE budget_id = p_original_budget_id;
  END IF;

  -- Copiar uniforms (function_id é text)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'uniforms') THEN
    ...
    FROM uniforms
    WHERE budget_id = p_original_budget_id AND function_id IS NOT NULL AND v_function_map ? function_id;
  END IF;
  ...
  -- Copiar differentiated_benefits (colunas corretas: name, monthly_value)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'differentiated_benefits') THEN
    ...
    SELECT v_new_budget_id, (v_function_map->>function_id), name, monthly_value
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'budget_calculations') THEN
    ...
    SELECT v_new_budget_id, function_data, total_contract, total_payroll, total_benefits,
           total_encargos, total_materials, total_equipments, total_uniforms,
           total_capex, total_others, total_differentiated_benefits
