/*
  # Criar tabela de uniformes de Compras
  ...
  2. Segurança
    - Habilitar RLS na tabela `purchases_uniforms`
...
CREATE TABLE IF NOT EXISTS purchases_uniforms (
  ...
  CONSTRAINT purchases_uniforms_unique_key UNIQUE (company, contract_name, month_ym)
);

ALTER TABLE purchases_uniforms ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated can select purchases_uniforms"
  ON purchases_uniforms
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Authenticated can insert purchases_uniforms"
  ON purchases_uniforms
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Authenticated can update purchases_uniforms"
  ON purchases_uniforms
  FOR UPDATE
  TO authenticated
  USING (true)
...
CREATE POLICY "Authenticated can delete purchases_uniforms"
  ON purchases_uniforms
  FOR DELETE
  TO authenticated
  USING (true);

CREATE INDEX IF NOT EXISTS idx_purchases_uniforms_month 
  ON purchases_uniforms(month_ym);

CREATE INDEX IF NOT EXISTS idx_purchases_uniforms_company 
  ON purchases_uniforms(company);

CREATE INDEX IF NOT EXISTS idx_purchases_uniforms_contract 
  ON purchases_uniforms(contract_name);
