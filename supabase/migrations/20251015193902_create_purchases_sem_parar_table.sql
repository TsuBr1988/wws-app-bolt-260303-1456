/*
  # Criar tabela de Sem Parar de Compras
  ...
  2. Segurança
    - Habilitar RLS na tabela `purchases_sem_parar`
...
CREATE TABLE IF NOT EXISTS purchases_sem_parar (
  ...
  CONSTRAINT purchases_sem_parar_unique_key UNIQUE (company, department, month_ym)
);

ALTER TABLE purchases_sem_parar ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated can select purchases_sem_parar"
  ON purchases_sem_parar
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Authenticated can insert purchases_sem_parar"
  ON purchases_sem_parar
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Authenticated can update purchases_sem_parar"
  ON purchases_sem_parar
  FOR UPDATE
  TO authenticated
  USING (true)
...
CREATE POLICY "Authenticated can delete purchases_sem_parar"
  ON purchases_sem_parar
  FOR DELETE
  TO authenticated
  USING (true);

CREATE INDEX IF NOT EXISTS idx_purchases_sem_parar_month 
  ON purchases_sem_parar(month_ym);

CREATE INDEX IF NOT EXISTS idx_purchases_sem_parar_company 
  ON purchases_sem_parar(company);

CREATE INDEX IF NOT EXISTS idx_purchases_sem_parar_department 
  ON purchases_sem_parar(department);
